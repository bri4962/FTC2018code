...
@Autonomous(name="Auto_BlueCornerKeyOnbot", group = "Auto")
public class Auto_BlueCornerKeyOnbot extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()){
            ...
            robot.DriveOnHeading (0,20,.2,.03);
            robot.StopDriving();
            robot.RotateToHeading(-90,.02);
            robot.DriveOnHeadingBackwards(-90,12,-.2,.03);
            robot.Drive (-.2,-.2);
            sleep (1000);
            
            if (whichColumn == "left" && whichColumn == "unknown") {
                robot.DriveOnHeading (-90,16,.2,.03);
            }
            
            if (whichColumn == "center") {
                robot.DriveOnHeading (-90,24,.2,.03);
            }
            
            if (whichColumn == "right") {
                robot.DriveOnHeading (-90,32,.2,.03);
            }
            
            robot.RotateToHeading(-180,.02);
            robot.DriveOnHeadingBackwards(-180,4,-.2,.03);
            robot.StopDriving();
            robot.tiltCage.setTargetPosition(200);
            robot.tiltCage.setPower(1);
            sleep (2000);
            robot.DriveOnHeading (-180,3,.2,.03);
            robot.StopDriving();
